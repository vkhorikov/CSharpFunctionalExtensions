...
namespace CSharpFunctionalExtensions.Tests.MaybeTests.Extensions
{
    public class ToUnitResultTests_Task : MaybeTestBase
    {
        ...
        [Fact]
        public async Task ToUnitResult_Task_returns_custom_failure_via_error_function_if_has_no_value()
        {
            Maybe<T> maybe = null;

            var result = await maybe.AsTask().ToUnitResult(ErrorFunc);
        ...
        [Fact]
        public async Task ToUnitResult_Task_custom_failure_with_error_function_returns_success_if_has_value()
        {
            var maybe = Maybe<T>.From(T.Value);
